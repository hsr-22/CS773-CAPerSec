...
#define MEASUREMENT_TIME_MS   10000      // 10 seconds total measurement time
...
#define CACHE_WAYS    16     // number of ways (associativity)
#define SETS_PER_PAGE 64     // eviction buffer is organized into 64 sets per page
#define SET_SKIPPING_STEP 20  // skip sets in probing (probe every 2nd set)
...
void perform_measurement(unsigned int *results) {
    ...
    while (now_ms() < period_start+10000) {
...
void perform_measurement_ori(unsigned int *results) {
    unsigned int sample = 0;
...
int main(){
    ...
    perform_measurement_ori(results);
    // double period_start = now_ms();
    // while (now_ms() < period_start+10000) {
    //     probe_all_sets();
    // }
    ...
    FILE *outfile = fopen("cache_probe_results_dup.txt", "w");
    if (outfile == NULL) {
        perror("fopen");
        free(eviction_array);
        free(results);
